...
A visitor opens Foxglove (app or embed) and **teleoperates a marine the way Foxglove teleoperates a robot**. A ViZDoom process is the fake robot. The Foxglove SDK WebSocket is the live connection. The marine's framebuffer is a camera topic. The WAD map is a 3D occupancy grid plus **linedef wall meshes** in the stock 3D panel (`/doom/walls`, workstream `05`) with a player transform. The Teleop panel publishes Twist. Fire and use are a second publish path. Every session can be recorded as MCAP and replayed in the same layout.

The picture must come through **stock Foxglove panels** in the embed iframe. A canvas that happens to sit next to Foxglove is a failed stunt. Host HTML chrome (WASD key bar, health/armor/ammo bars) beside the iframe is allowed; it is not a `.foxe` and it is not a framebuffer canvas.
...
- `dec_stock_panels_only` — The Foxglove **iframe** stays stock panels only (Image, 3D, Teleop, Gauge, Plot, Log, Raw Messages, Table). Human-reopened 2026-09-18: HUD chrome is **not** a `.foxe`. See `dec_host_html_hud`.
- `dec_host_html_hud` (SUPERSEDES the old “no custom HUD until gauges fail” reading of `dec_stock_panels_only`) — HUD chrome (WASD keys + health/armor/ammo bars) is **host HTML** beside the iframe. `@foxglove/embed` `installExtensions` is **self-hosted only**; this kiosk uses `https://embed.foxglove.dev/`, so a `.foxe` will not load. Custom `.foxe` remains out until self-hosted embed. `dec_no_canvas` still holds for the marine image (no host `<canvas>` / `getContext` framebuffer). Gauge panels in Play/Debug/Replay are optional; extra Gauges do not fail the stunt.
...
- [ ] `cap_3d_map_hud` — `/doom/map` grid, `/tf` player transform, `/doom/entities` SceneUpdate, `/doom/log`, exported `Play` and `Debug` layouts. Health/armor/ammo Gauges are optional (`dec_host_html_hud`); host HTML bars are `05`.
...
- [ ] `cap_stunt_extras` — Garnish owned by workstream `05-stunt-extras` (not a workstream `06-*`): tagged events, copy-paste agent prompts, and **demo UX polish** (host WASD HUD, player SceneUpdate mesh, host HTML health bars, Play starts a **new** game, replay file browser, **linedef walls in stock ThreeDee**). Comparison, remote gateway, and cloud upload stay later extras. Not started until the first three capabilities are `done`.
...
- `/doom/walls` — `foxglove.SceneUpdate` linedef wall meshes once per level (host `05` UX polish; occupancy Grid stays)
...
- Custom `.foxe` in the embed iframe (host HTML HUD is allowed)
- Mouse-look, true FPS aiming, multiplayer deathmatch
- Commercial IWAD redistribution
- A from-scratch IWAD lump parser (`struct.unpack` of WAD directories). Read ViZDoom `get_state()` linedefs/sectors (`dec_vizdoom_engine`).
- Textured WAD rendering, full sector-floor meshes, or a host-canvas map view
